...
[System.Serializable]
[CreateAssetMenu(menuName = "Definitions/Game Action/VFX/ArcVFXActionBlock")]
public class ArcVFXAction : VFXActionBlock
{
    public override void Invoke(string[] idParams, List<object> currentFilteredTargets, Dictionary<string, object> otherTargets) {
        /*---------------------------------------------------------------------------------
            ...
            idParams[1] is guaranteed to be a Vector3 point for ArcVFXAction if the 
            user has entered the key correctly in the ability editor.
        ---------------------------------------------------------------------------------*/

        // Vector2 spawnPoint = (Vector3) otherTargets[idParams[1]]; 
        // ParticleSystem vfx = Instantiate<ParticleSystem>( Resources.Load<ParticleSystem>(libraryPrefix + "ArcVFX/" + idParams[0]), spawnPoint, Quaternion.identity );
        // vfx.Play();
    }
}
